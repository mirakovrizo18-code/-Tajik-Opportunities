...
interface ReviewAggregateRow {
  ...
  anonymous: string | number | null;
  featured: string | number | null;
  pinned: string | number | null;

  rating_sum: string | number | null;
  rating_1: string | number | null;
  rating_2: string | number | null;
  rating_3: string | number | null;
  rating_4: string | number | null;
  rating_5: string | number | null;
}
...
const REVIEW_COUNTER_FIELDS: readonly ReviewCounterField[] = [
  "views_count",
  "helpful_count",
  "not_helpful_count",
  "reactions_count",
...
const REPLY_COUNTER_FIELDS: readonly ReplyCounterField[] = [
  "reactions_count",
  "reports_count",
  "replies_count",
  "views_count",
...
const REVIEW_STATUS_VALUES = new Set<string>(
  Object.values(REVIEW_STATUSES).map(String),
);

const REVIEW_VISIBILITY_VALUES = new Set<string>(
  Object.values(REVIEW_VISIBILITY).map(String),
);

const REVIEW_AUTHOR_MODE_VALUES = new Set<string>(
  Object.values(REVIEW_AUTHOR_MODES).map(String),
);
...
function validateRating(
  rating: unknown,
): asserts rating is ReviewRating {
...
function validateAuthorMode(
  value: unknown,
): asserts value is ReviewAuthorMode {
  if (
    !isNonEmptyString(value) ||
    !REVIEW_AUTHOR_MODE_VALUES.has(value)
  ) {
    throw new ValidationError(
      "Некорректный режим автора",
    );
  }
}
...
function validateVisibility(
  value: unknown,
): asserts value is ReviewVisibility {
  if (
    !isNonEmptyString(value) ||
    !REVIEW_VISIBILITY_VALUES.has(value)
  ) {
    throw new ValidationError(
      "Некорректная видимость отзыва",
    );
  }
}
...
function validateStatus(
  ...
  if (
    !isNonEmptyString(value) ||
    !REVIEW_STATUS_VALUES.has(value)
  ) {
    throw new ValidationError(
...
function validateCounterField(
  field: unknown,
): asserts field is ReviewCounterField {
  if (
    typeof field !== "string" ||
    !REVIEW_COUNTER_FIELDS.includes(
      field as ReviewCounterField,
    )
  ) {
    throw new ValidationError(
      "Недопустимое поле счётчика",
    );
  }
}

function validateReplyCounterField(
  field: unknown,
): asserts field is ReplyCounterField {
  if (
    typeof field !== "string" ||
    !REPLY_COUNTER_FIELDS.includes(
      field as ReplyCounterField,
    )
  ) {
    throw new ValidationError(
      "Недопустимое поле счётчика ответа",
    );
  }
}
...
function mapReview(
  ...
  return {
    ...
    views_count: positiveDecimal(
      row.views_count,
    ),
    helpful_count: positiveDecimal(
      row.helpful_count,
    ),
    not_helpful_count: positiveDecimal(
      row.not_helpful_count,
    ),
    reactions_count: positiveDecimal(
      row.reactions_count,
    ),
    reports_count: positiveDecimal(
      row.reports_count,
    ),
    replies_count: positiveDecimal(
      row.replies_count,
    ),
    shares_count: positiveDecimal(
      row.shares_count,
    ),
  };
}
...
function mapMetrics(
  row: ReviewMetricsRow,
): ReviewMetrics {
  return {
    ...row,

    total_reviews: positiveDecimal(
      row.total_reviews,
    ),
    published_reviews: positiveDecimal(
      row.published_reviews,
    ),
    pending_reviews: positiveDecimal(
      row.pending_reviews,
    ),
    hidden_reviews: positiveDecimal(
      row.hidden_reviews,
    ),
    rejected_reviews: positiveDecimal(
      row.rejected_reviews,
    ),
    deleted_reviews: positiveDecimal(
      row.deleted_reviews,
    ),

    total_ratings: positiveDecimal(
      row.total_ratings,
    ),
    rating_sum: positiveDecimal(
      row.rating_sum,
    ),
    rating_average: positiveDecimal(
      row.rating_average,
    ),

    rating_1: positiveDecimal(
      row.rating_1,
    ),
    rating_2: positiveDecimal(
      row.rating_2,
    ),
    rating_3: positiveDecimal(
      row.rating_3,
    ),
    rating_4: positiveDecimal(
      row.rating_4,
    ),
    rating_5: positiveDecimal(
      row.rating_5,
    ),

    total_reactions: positiveDecimal(
      row.total_reactions,
    ),
    total_helpful: positiveDecimal(
      row.total_helpful,
    ),
    total_not_helpful: positiveDecimal(
      row.total_not_helpful,
    ),

    total_reports: positiveDecimal(
      row.total_reports,
    ),
    total_replies: positiveDecimal(
      row.total_replies,
    ),
    total_views: positiveDecimal(
      row.total_views,
    ),

    verified_reviews: positiveDecimal(
      row.verified_reviews,
    ),
    anonymous_reviews: positiveDecimal(
      row.anonymous_reviews,
    ),
    featured_reviews: positiveDecimal(
      row.featured_reviews,
    ),
    pinned_reviews: positiveDecimal(
      row.pinned_reviews,
    ),
...
function ratingCount(
  ...
  switch (rating) {
    case 1:
      return metrics.rating_1;

    case 2:
      return metrics.rating_2;

    case 3:
      return metrics.rating_3;

    case 4:
      return metrics.rating_4;

    case 5:
      return metrics.rating_5;
...
function calculateRatingAverage(
  ratingSum: DecimalString,
  totalRatings: DecimalString,
): DecimalString {
  if (
    compareDecimalStrings(
      totalRatings,
...
export class ReviewService {
  ...
  async getById(
    ...
    const reviewId = validateId(
      id,
      "Не указан ID отзыва",
    );

    const row =
      await this.db.first<ReviewRow>(
        `
        SELECT *
        FROM reviews
        WHERE id = ?
  ...
  async adminUpdate(
    ...
    for (
      const field of REVIEW_COUNTER_FIELDS
    ) {
      const value =
        input[field];
  ...
  async setHelpful(
    ...
    const field: ReviewCounterField =
      helpful
        ? "helpful_count"
        : "not_helpful_count";
  ...
  async incrementCounter(
    id: string,
    field: ReviewCounterField,
    amount: DecimalString = "1",
  ): Promise<Review> {
    validateCounterField(field);
  ...
  async getReplies(
    ...
    const rows =
      await this.db.query<ReviewReplyRow>(
        `
        SELECT *
        FROM review_replies
        WHERE review_id = ?
  ...
  async addReaction(
    ...
    if (!actorId) {
      throw new ValidationError(
        "Не указан пользователь или посетитель",
      );
    }
  ...
  async createReport(
    ...
    const report: ReviewReport = {
      ...
      reporter_visitor_id:
        input.reporter_visitor_id ??
        null,

      reporter_session_id:
        input.reporter_session_id ??
        null,
    ...
    const created =
      await this.db.first<ReviewReportRow>(
        `
        SELECT *
        FROM review_reports
        WHERE id = ?
  ...
  async recalculateMetrics(
    ...
    const counts =
      await this.db.first<ReviewAggregateRow>(
        `
        SELECT
          COUNT(*) AS total,
     ...
     *
     * rating_sum / total_ratings
     *
     * В отличие от старой реализации здесь нет
     * лишнего weighted-выражения.
     */
    const ratingAverage =
  ...
  async getRatingSummary(
    ...
    /*
     * Ключи намеренно приводятся к структуре
     * ReviewRatingDistribution из types/review.
     */
    const distribution =
      ({
        1: metrics.rating_1,
        ...
        4: metrics.rating_4,
        5: metrics.rating_5,
      } as unknown) as ReviewRatingDistribution;
  ...
  > {
    ...
    for (
      const reviewId of input.review_ids
    ) {
      try {
        const review =
          await this.requireById(
  ...
  private async incrementReplyCounter(
    id: string,
    field: ReplyCounterField,
  ): Promise<void> {
    validateReplyCounterField(
      field,
    );
